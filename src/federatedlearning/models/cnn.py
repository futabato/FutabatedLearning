...
class CNNCifar(nn.Module):
    def __init__(self, cfg: DictConfig) -> None:
        """
        A Convolutional Neural Network designed for the CIFAR dataset.

        Args:
            cfg (DictConfig): A configuration object specifying hyperparameters.
        """
        super(CNNCifar, self).__init__()
        # Convolutional layer taking in 3 input channels for RGB images and outputting 6 channels
        self.conv1 = nn.Conv2d(3, 6, 5)
        # Max pooling layer with kernel size 2 and stride 2 to reduce spatial dimensions
        self.pool = nn.MaxPool2d(2, 2)
        # Second convolutional layer with 6 input channels and 16 output channels
        self.conv2 = nn.Conv2d(6, 16, 5)
        # Fully connected layers to transform the features to class scores
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        # Apply the final fully connected layer that outputs class scores
        x = self.fc3(x)
        # Apply log softmax to obtain log probabilities for classification
        return F.log_softmax(x, dim=1)
